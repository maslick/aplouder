...
Aplouder.prototype.processFiles = function (files, callback) {
    var self = this;
    for (var i = 0; i < files.length; i++) {
        var src = files[i];
        Aplouder.file2base64(src, function (original, file64) {
            Aplouder.scaleImage(file64 || self.unknown, 120, 120, function (scaledImg) {
                var obj = { src: original, base64: file64 || self.unknown, thumb64: scaledImg, i: self.numberOfFiles++ };
...
Aplouder.prototype.removeGallery = function () {
...
Aplouder.prototype.initBrowseFilesButton = function () {
    var self = this;
    document.querySelector("." + self.id + ".ap-droparea > .ap-message").onclick = function () {
...
Aplouder.prototype.init = function () {
    ...
    function handlePrev() {
        if (self.slideNumber !== null) {
            var prevSlide = self.slideNumber - 1;
    ...
    function handleNext() {
        if (self.slideNumber !== null) {
            var nextSlide = self.slideNumber + 1;
...
Aplouder.scaleImage = function (url, width, height, callback) {
    var img = new Image();
    ...
    img.onload = function () {
        ...
        if (img.width === img.height)
            ...
            var minVal = Math.min(img.width, img.height);
            if (img.width > img.height)
...
Aplouder.file2base64 = function (file, callback) {
...
Aplouder.drawImage = function (id, json) {
    var onclick = "Aplouder.openModal('" + id + "'); Aplouder.currentSlide('" + id + "', " + json.i + ");";
...
Aplouder.formatFileSize = function (bytes) {
...
Aplouder.addSlide = function (id, f) {
...
Aplouder.hideModal = function (id) {
    document.querySelector("." + id + ".ap-modal").style.display = "none";
    var slides = document.querySelectorAll("." + id + ".ap-modal > .ap-modal-content > .ap-slides");
    for (var i = 0; i < slides.length; i++) slides[i].style.display = "none";
    document.getElementsByTagName("body")[0].style.overflow = "auto";
    Aplouder.aplouders.filter(function (v) { return v.id === id; })[0].slideNumber = null;
};

Aplouder.openModal = function (id) {
    document.querySelector("." + id + ".ap-modal").style.display = "block";
    document.getElementsByTagName("body")[0].style.overflow = "hidden";
};

Aplouder.currentSlide = function (id, n) {
    var slides = document.querySelectorAll("." + id + ".ap-modal > .ap-modal-content > .ap-slides");
    for (var i = 0; i < slides.length; i++) slides[i].style.display = "none";
    slides[n].style.display = "block";
    Aplouder.aplouders.filter(function (v) { return v.id === id; })[0].slideNumber = n;
};