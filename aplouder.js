class Aplouder {
    ...
    init() {
        ...
        const template_html =
            ...
            '   </div>\n' +
            '   <div id="ap-gallery"></div>\n' +
            '</div>\n' +
            '<div id="ap-modal" class="ap-modal" onclick="Aplouder.hideModal()">\n' +
        ...
        function handlePrev() {
            if (Aplouder.slideNumber !== null) {
                const prevSlide = Aplouder.slideNumber - 1;
                if (prevSlide !== -1) Aplouder.currentSlide(prevSlide);
                else Aplouder.currentSlide(Object.keys(self.Filez).length - 1);
            }
        }

        function handleNext() {
            if (Aplouder.slideNumber !== null) {
                const nextSlide = Aplouder.slideNumber + 1;
                if (nextSlide !== Object.keys(self.Filez).length) Aplouder.currentSlide(nextSlide);
    ...
    drawImage(json) {
        let onclick = 'Aplouder.openModal(); Aplouder.currentSlide(' + json.i + ');';
        this.getGalleryEl().innerHTML +=
            ...
            '           <div class="ap-size">' +
                            this.formatFileSize(json.src.size) +
            '           </div>' +
            '           <div class="ap-name">' +
                            json.src.name +
            '           </div>' +
    ...
    addSlide(f) {
        ...
        document.getElementsByClassName("ap-modal-content")[0].innerHTML +=
            ...
            '   <div class="ap-slide-size">' +
                    this.formatFileSize(f.src.size) +
            '   </div>' +
            '</div>';
    };
    ...
    static currentSlide(n) {
        let slides = document.getElementsByClassName("ap-slides");
        for (let i = 0; i < slides.length; i++) slides[i].style.display = "none";
